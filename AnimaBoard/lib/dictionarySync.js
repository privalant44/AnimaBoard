...
async function ensureResourceDictionary(resources) {
  ...
  if ((needsSync || hasUnresolvedLabels) && Date.now() - lastAutoSyncMs >= AUTO_SYNC_COOLDOWN_MS) {
    lastAutoSyncMs = Date.now();
    try {
      console.log('[dictionarySync] Dictionnaire incomplet — synchronisation Boond…');
...
async function getResourcesLocalEnriched() {
  ...
  return enrichResourcesWithLabels(baseList, maps);
}
