...
function getRequiredPermissions(method, pathname) {
  ...
  if (path.startsWith('/api/auth/role-permissions')) return [PERMISSIONS.USERS_MANAGE];
  ...
  if (path === '/api/settings/logo' || path.endsWith('/settings/logo')) {
  ...
  if (m === 'POST' || m === 'PUT' || m === 'PATCH' || m === 'DELETE') {
    ...
    if (path.includes('/api/data/planned-deliveries')) return [PERMISSIONS.DATA_WRITE];
