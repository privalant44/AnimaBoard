...
const APP_TABS = Object.freeze(['home', 'resources', 'forecast', 'report', 'settings']);

function canAccessTab(permissions, tab) {
  const perms = permissions || [];
  if (tab === 'home') {
    return (
      perms.includes(PERMISSIONS.TAB_HOME) ||
      perms.includes(PERMISSIONS.VIEW_HOME_FINANCIAL) ||
      perms.includes(PERMISSIONS.VIEW_HOME_BESOINS) ||
      perms.includes(PERMISSIONS.VIEW_HOME_TREASURY)
    );
  }
  if (tab === 'forecast') {
    return (
      perms.includes(PERMISSIONS.TAB_FORECAST) ||
      perms.includes(PERMISSIONS.VIEW_FORECAST_PERSONAL) ||
      perms.includes(PERMISSIONS.VIEW_FORECAST_SCENARIOS)
    );
  }
  if (tab === 'report') {
    return (
      perms.includes(PERMISSIONS.TAB_REPORT) ||
      perms.includes(PERMISSIONS.VIEW_REPORT_FORECAST) ||
      perms.includes(PERMISSIONS.VIEW_REPORT_INCOME)
    );
  }
  if (tab === 'resources') return perms.includes(PERMISSIONS.TAB_RESOURCES);
  if (tab === 'settings') return perms.includes(PERMISSIONS.TAB_SETTINGS);
  return false;
}
