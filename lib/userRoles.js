...
const { ROLES, isValidRole, getRoleLabel } = require('./roles');
const { getEffectivePermissionsForRole } = require('./rolePermissions');
...
async function resolveUserAccess(authPayload) {
  ...
  if (isLocalAdminAuth(authPayload)) {
    return await buildAccess(email, LOCAL_ADMIN_ROLE);
  }

  const overrides = parseRoleOverrides();
  if (overrides.has(email)) {
    return await buildAccess(email, overrides.get(email));
  }

  const cached = roleCache.get(email);
  if (cached && Date.now() - cached.at < CACHE_TTL_MS) {
    return await buildAccess(email, cached.role);
  }

  const fromDb = await fetchRoleFromSupabase(email);
  const role = fromDb || getDefaultRole();
  roleCache.set(email, { role, at: Date.now() });
  return await buildAccess(email, role);
}

async function buildAccess(email, role) {
